...
<div class="mobile">
    ...
    <button type="button" class="btn_purple mb-3" data-bs-toggle="modal" data-bs-target="#add_pengeluaran_ps">
        Add Pengeluaran Ps
    </button>
    ...
    <?php else : ?>
        <?php foreach ($data as $i) : ?>
            <div class="div_list <?= ($i['jenis'] == 'Inv' ? 'bg_success_bright' : ''); ?>">
                <div class="d-flex justify-content-between">
                    <div class="detail" data-order="<?= $i['jenis']; ?>" data-id="<?= $i['id']; ?>"><?= $i['barang']; ?></div>
            ...
            <div class="modal-body">
                ...
                <form action="<?= base_url(menu()['controller']); ?>/add" method="post">
                    ...
                    <div class="mb-2">
                        ...
                        <input class="input" type="number" name="qty" placeholder="Jumlah" required>
                    </div>
                    <div class="mb-2">
...
<div class="modal fade" id="add_pengeluaran_ps" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <div class="d-flex justify-content-center body_select d-none">
                ...
                <h6 class="text_main2 fw-bold"> <i class="fa-solid fa-user"></i> Add <?= menu()['menu']; ?></h6>
                <hr>
                <form action="<?= base_url(menu()['controller']); ?>/add_pengeluaran_ps" method="post">
                    <div class="mb-2">
                        <div class="text_main2">Tgl</div>
                        <input type="date" class="input" name="tgl" data-date-format="DD/MM/YYYY" value="<?= date('Y-m-d'); ?>">
                    </div>
                    <div class="mb-2">
                        <div class="text_main2">Pengeluaran</div>
                        <input class="input" type="text" name="barang" placeholder="Keperluan" required>
                    </div>
                    <div class="mb-2">
                        <div class="text_main2">Qty</div>
                        <input class="input" type="number" name="qty" placeholder="Jumlah" required>
                    ...
                    <div class="mb-2">
                        <div class="text_main2">Keterangan</div>
                        <textarea class="input" name="ket" placeholder="Keterangan" rows="3"></textarea>
                    </div>
                    <div class="d-grid">
                        <button type="submit" class="btn_primary"><i class="fa-solid fa-cloud"></i> Save</button>
                    </div>
...
<script>
    $(document).on('click', '.detail', function(e) {
        ...
        if (order == 'Inv') {
            ...
            html += '<input class="input" type="hidden" name="id" value="' + data.id + '" placeholder="No. Inv" readonly>';
        }
        html += '<div class="mb-2">';
        ...
        if (order == 'Inv') {
            ...
            html += '<div class="mb-2">';
            html += '<div class="text_main2">Pengeluaran</div>';
            html += '<input class="input" type="text" name="barang" value="' + data.barang + '" placeholder="Keperluan" required>';
            html += '</div>';

        }
        html += '<div class="mb-2">';
        html += '<div class="text_main2">Qty</div>';
        html += '<input class="input" type="number" name="qty" value="' + data.qty + '" placeholder="Jumlah" required>';
        html += '</div>';
        html += '<div class="mb-2">';
        ...
        if (order == 'Inv') {
            ...
            html += '<input class="input" type="text" name="pembeli" value="' + data.pembeli + '" placeholder="Nama Pj" required>';
            html += '</div>';
