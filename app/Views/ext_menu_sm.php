...
<nav class="fixed-bottom">
    <div class="card py-3 body_btn_pesan" style="background-color: transparent;border:none">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body bg_3 rounded" style="padding: 1px;">
                <div class="bg_2 rounded p-0 body_daftar_order_sm">
...
<script>
    ...
    const daftar_order_sm = (data = []) => {

        let html = '';

        html += '<div class="d-flex justify-content-between p-3 border-bottom rounded" style="background-color: #db8600;">';
        html += '<div class="fw-bold text-light" style="font-size: medium;"><i class="fa-solid fa-bowl-rice"></i> Daftar Pesanan</div>';
        html += '<div><a style="color: #1b120c;font-size:medium" data-bs-dismiss="modal" href=""><i class="fa-solid fa-circle-xmark"></i></a></div>';
        html += '</div>';
        ...
        let total = 0;
        order_list_sm.forEach((e, i) => {

            html += '<tr>';
            html += '<td>' + (i + 1) + '</td>';
            html += '<td>' + e.barang + '</td>';
            html += '<td>' + e.qty_item + '</td>';
            html += '<td style="text-align: right;">' + angka(e.total_item) + '</td>';
            html += '</tr>';
        ...
        html += '<button style="background-color:#1b120c" class="rounded text-light p-2 btn_save_pesanan_sm"><i class="fa-solid fa-fire-burner"></i> Kirim Pesanan</button>';
        html += '</div>';
        html += '</div>';

        $('.body_daftar_order_sm').html(html);

        let myModal = document.getElementById('modal_pesanan');
    ...
    const btn_pesan = () => {
        let html = '';
        if (order_list_sm.length == 0) {
            $('.body_btn_pesan').html("");
        } else {
            html += '<div class="card-body">';
            html += '<div class="d-grid">';
            html += '<button style="background-color: #db8600;border:1px solid #1b120c;font-size:x-large" class="fw-bold text-white daftar_order_sm p-3 rounded"><i class="fa-solid fa-bowl-rice"></i> Pesan</button>';
            html += '</div>';
            html += '</div>';

            $('.body_btn_pesan').html(html);

        }
    }

    $(document).on('click', '.menu_list_sm', function(e) {
        e.preventDefault();
        ...
        if (exist > 0) {
            ...
            order_list_sm.forEach(e => {
                if (e.id == id) {
                    e.qty_item = e.qty_item + 1;
                    e.total_item = e.qty_item * harga;

                    let html = '';
                    html += '<div class="d-flex">';
                    html += '<a class="change_qty_sm" data-order="minus" data-id="' + id + '" data-harga="' + harga + '" style="font-size:medium;text-decoration: none;color:#fffefd" href=""><i class="fa-solid fa-circle-minus"></i></a>';
                    html += '<div style="color: #fffefd;border-bottom:1px solid #fffefd" class="mx-1 qty_order_' + id + '">' + e.qty_item + '</div>';
                    html += '</div>';
            ...
            html += '<div class="d-flex">';
            html += '<a class="change_qty_sm" data-order="minus" data-id="' + id + '" data-harga="' + harga + '" style="font-size:medium;text-decoration: none;color:#fffefd" href=""><i class="fa-solid fa-circle-minus"></i></a>';
            html += '<div style="color: #fffefd;border-bottom:1px solid #fffefd" class="mx-1 qty_order_' + id + '">1</div>';
            html += '</div>';

            $('.list_' + id).html(html);
            order_list_sm.push(item);
        }
    ...
    $(document).on('click', '.change_qty_sm', function(e) {
        e.preventDefault();
        ...
        order_list_sm = data_temp;
        btn_pesan();
    })

    $(document).on('click', '.daftar_order_sm', function(e) {
        e.preventDefault();
        daftar_order_sm();
    })

    $(document).on('click', '.btn_save_pesanan_sm', function(e) {
        e.preventDefault();
